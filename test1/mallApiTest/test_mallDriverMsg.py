...
4.获取司机等级，公里数，积分等信息
接口方法： GET
接口地址：driver/info?driverId=1589
'''
from test1.send_method import *
api = 'driver/info?driverId='#按需更改
...
if error == []:
    ...
    with open('./mallApiTest/error.txt',"a+",encoding="utf-8") as f:
        f.write('获取司机等级，公里数，积分等信息的接口，出现“系统错误，请联系管理员”返回结果如下：'+"\n" + str(error) + "\n" + "===============" + "\n")  # 按需更改汉字

with open('./mallApiTest/report.txt', 'a+', encoding="utf-8") as f:
    f.write('获取司机等级，公里数，积分等信息的接口，成功运行%s次，成功与失败返回如下：'%ok+"\n"+str(L)+"\n"+"==============="+"\n")  # 按需更改汉字